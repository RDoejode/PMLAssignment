...
The training data has 19,622 observations with 160 different variables. The variable <b>"classe"</b> has the way (A-E) the lifting was performed. A whole bunch of variables have only NAs for value. It would make sense to just drop all the columns that have just the NA values. Columns 1 through 7 have data that is not needed with the analysis. The code below gets rid of NAs filled columns and columns 1 through 7.
...
```
Conditional Inference tree yielded an accuracy of 0.8817. 
 
## Random Forest 
We will now use random forest to see if accuracy of prediction could be improved upon.
```{r}
...
### Submission - Applying the model on the final quiz data
Things cannot get any better than an accuracy of 0.9976 and we will stop here and use the model on the testing_Quiz data to find the <b>"classe"</b> for this test dataset.
```{r}
dim(testing_Quiz)
```
